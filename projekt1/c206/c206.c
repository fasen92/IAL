...
void DLL_Dispose( DLList *list ) {
	if(list->firstElement != NULL){
		DLLElementPtr delElem = list->firstElement;
		DLLElementPtr next;

		do{
			next = delElem->nextElement; //ulozi sa pointer na nasledujuci prvok
			free(delElem); //aktualny sa uvolni
			delElem = next;
		}while(delElem != NULL);
...
void DLL_DeleteFirst( DLList *list ) {
	if(list->firstElement != NULL){
		...
		if(list->firstElement != list->lastElement){
			DLLElementPtr nextEl = list->firstElement->nextElement;
			free(list->firstElement);
			list->firstElement = nextEl;
...
void DLL_DeleteLast( DLList *list ) {
	if(list->lastElement != NULL){
		...
		if(list->lastElement != list->firstElement){
			DLLElementPtr lastEl = list->lastElement;
...
void DLL_DeleteAfter( DLList *list ) {
	if(list->activeElement != list->lastElement && list->activeElement != NULL){
		if(list->activeElement->nextElement != list->lastElement){
			DLLElementPtr nextEl = list->activeElement->nextElement->nextElement; 
			free(list->activeElement->nextElement);
			nextEl->previousElement = list->activeElement;
			list->activeElement->nextElement = nextEl;
		}else{
			free(list->activeElement->nextElement);
...
void DLL_DeleteBefore( DLList *list ) {
	if(list->activeElement != list->firstElement && list->activeElement != NULL){
		if(list->activeElement->previousElement != list->firstElement){
			DLLElementPtr prevEL = list->activeElement->previousElement->previousElement; 
			free(list->activeElement->previousElement);
			prevEL->nextElement = list->activeElement;
			list->activeElement->previousElement = prevEL;
		}else{
			free(list->activeElement->previousElement);
